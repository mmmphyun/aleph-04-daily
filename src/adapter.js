...
/**
 * ALEPH T04 정규화 및 상태 전이 엔진
 * 계약: public-contract.json, normalized-reading.schema.json, reading-status.schema.json
 */
...
export function applyError(inputState, errorCode, runMeta = {}) {
  ...
  const state = clone(inputState);
  // C17: 실패 뒤 마지막 정상값이 지워지지 않는다 (current_reading 유지)
  // C18: 실패 뒤 마지막 정상값에 오래된 값(stale) 표시가 붙는다
...
export function runFixture(inputState, fixture) {
  const meta = {
    ...
    retry_after_seconds: fixture.transport.headers && fixture.transport.headers['retry-after']
